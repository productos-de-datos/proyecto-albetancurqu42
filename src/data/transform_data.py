...
def get_index_col_of_value(df_source, value="Fecha"):
    """Encuentra el primer valor valido del archivo Excel"""
...
def clean_dataframe(
    df_source, pos_col_value, pos_index_value, value="Fecha", limit_hours=24
):
    """Limpia el dataframe original"""
...
def transform_data(
    source_path=landing_path, target_path=raw_path, corner_value="Fecha"
):
    """Transforma los archivos xls a csv.

    Transforma los archivos data_lake/landing/*.xls a data_lake/raw/*.csv. Hay
    ...
    for excel_file in list_excel_files:
        ...
        pos_col_value, pos_index_value = get_index_col_of_value(
            df_raw, value=corner_value
        )
